...
class NetworkEmulator(object):
  ...
  def emulate(self, target_ip,set_way):
    ...
    if (set_way == 1 or set_way == 0):
        incoming_rule_id = self._create_dummynet_rule(receive_pipe_id, 'any',target_ip, self._port_range)
        logging.debug('Created incoming rule: %s', incoming_rule_id)
    if (set_way == 2 or set_way == 0):
        outgoing_rule_id = self._create_dummynet_rule(send_pipe_id, target_ip,'any', self._port_range)
  ...
  def _create_dummynet_rule(self, pipe_id, from_address, to_address,
    ...
    print "to_address is " , to_address;
    print "from_address is " , from_address;
    _run_ipfw_command(add_part ,
