...
///////////////////////////////////////////////////changed.
  /*  var div = document.getElementById('test' + streamId);
    ...
    var div = document.getElementById('mixed-streams'); 
    div.setAttribute('style', 'width: ' + resolution.width + 'px; height: ' +
      resolution.height + 'px;');
    stream.show('mixed-streams');
  }
    ...
    if (stream instanceof Woogeen.RemoteMixedStream) {
     ...
     //   width: 640,
       // height: 480
        ...
        console.log("resolution is ________________", resolution);
          videoOpt = {
            resolution: resolution
          };
          L.Logger.info('subscribe stream with option:', resolution);
            ...
            console.log("resolution2 --- is ________________", resolution);
    ...
    for (var i in conference.localStreams) {
      if (conference.localStreams.hasOwnProperty(i)) {
        if (conference.localStreams[i].id() === stream.id()) {
          localStreamId = conference.localStreams[i].id();
  ...
  window.onload = function() {
    ...
    var myResolution = getParameterByName('resolution') || 'hd720p';
      ...
      conference.join(token, function(resp) {
        if (typeof mediaUrl === 'string' && mediaUrl !== '') {
          Woogeen.ExternalStream.create({
            ...
            localStream = stream;
            conference.publish(localStream, {videoCodec:'vp8'}, function(st) {
