...
def extrace_freeze(input_file,max_drop_count,hi,lo,frac,freeze_threshold):
  #Only support encoded video analysis currently 
    ...
    for _i in _lines:     
        ...
        if 'drop_count' in _i and 'pts_time' in _i:
            ...
            if int(_value) > 0:
                ...
                if(_freeze_mark == 0):                   
                   _freeze_mark = 1   
                   _freeze_length = float(_time_stamp)-float(_last_freeze_time_stamp)
    ...
    freeze_rate = (float(_input_frame_num)-float(_output_frame_num))/float(_input_frame_num)
...
def main(argv):
    ...
    g_fps_value,g_inital_frame_time,g_freeze_list, g_freeze_length_list = extrace_freeze(g_input,max_drop_count,hi,lo,frac,freeze_threshold)
    os.chdir(targ_cwd)
